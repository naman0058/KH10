...
router.post('/',(req,res)=>{
  ...
  router.post("/mycart", (req, res) => {
    ...
    var query = `select c.*,(select s.name from services s where s.id = c.booking_id) as servicename,
                 ...
                 (select m.quantity from menu_manage m where m.productid = c.booking_id ) as productweight,
                 (select m.price from menu_manage m where m.productid = c.booking_id) as productprice
...
router.post("/cart-handler", (req, res) => {
  ...
  pool.query(`delete from cart where booking_id = '${req.body.booking_id}' and  usernumber = '${req.body.usernumber}' `,(err,result)=>{
      ...
      pool.query(`select oneprice from cart where booking_id = '${req.body.booking_id}' and  weight = '${req.body.weight}' and usernumber = '${req.body.usernumber}'`,(err,result)=>{
          if (err) throw err;
          else if (result[0]) {
             // res.json(result[0])
              pool.query(`update cart set quantity = ${req.body.quantity} , price = ${result[0].oneprice}*${req.body.quantity}  where booking_id = '${req.body.booking_id}' and weight = '${req.body.weight}' and usernumber = '${req.body.usernumber}'`,(err,result)=>{
                  if (err) throw err;
                  else {
                      res.json({
                        msg: "updated sucessfully",
